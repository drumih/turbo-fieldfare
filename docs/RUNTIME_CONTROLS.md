...
| Routed experts per token | Not exposed | `--experts-per-token 4\|6\|8` | 8 | Fewer experts cut the routed computation and the expert reads each token needs, at a quality cost that depends on the prompt. 8 is the checkpoint's own routing width and the only width used in benchmarks. |
