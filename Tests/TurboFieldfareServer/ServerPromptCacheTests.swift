...
struct ServerPromptCacheTests {
    ...
    /// The prompt-cache domain used to cover configuration fields only and
    ...
    @Test func runtimeIdentityCoversTheSwitchesThatSelectKernels() {
        ...
        let unprotected = RuntimeConfiguration(prefillWatchdogProtectionEnabled: false)
        #expect(ServerModelSession.runtimeIdentityString(
            runtime: unprotected, environment: [:]) != plain,
            "watchdog mode did not change cache identity")
